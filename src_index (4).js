...
function dynamicEpsilonFromHist(histArray, envEps = MACD_EPSILON) {
  ...
  // choose relative fraction; make it more sensitive by using 5% of median instead of 10%
  return Math.max(1e-8, med * 0.05);
}
...
function isNegToPosFlexible(prev, cur, histSample, opts = {}) {
  ...
  const eps = dynamicEpsilonFromHist((histSample || []).map(h => (typeof h === 'object' ? Number(h.hist || 0) : Number(h || 0))));
 ...
 *   closes, macd object, and histSample (array of { ts, hist } for lastN)
 */
async function fetchHistPrevAndCur(symbol, tfIntervalKey, klinesLimit = 500) {
  ...
  // make sure we request at least a moderate number of bars
  const want = Math.max(80, Math.min(1500, Number(klinesLimit || 500)));

  if (REST_THROTTLE_MS > 0) await sleep(REST_THROTTLE_MS);
  const raw = await by.fetchKlines(symbol, tfIntervalKey, want);
...
async function fallbackRootScan(rootTf='1h', symbolLimit=MAX_SCAN_SYMBOLS) {
  ...
  const immediateFlips = results.filter(r => isNegToPosFlexible(r.prevClosedHist, r.currentHist, r.histSample));
...
async function confirmCandidateMtf(symbol, rootTf, maxWaitSeconds) {
  ...
  let lastDetails = [];
  // use POLL_INTERVAL_MS from environment/default above; cap to safe range
...
async function performRootScan(rootTf='1h', symbolLimit=MAX_SCAN_SYMBOLS) {
  ...
  const confirmTasks = immediateFlips.map(c => limiter(async () => {
    // shorten confirmation window to at most one root interval (helps re-check quickly)
    const confirmMaxWait = Math.min(maxWait, rootSec);
    const res = await confirmCandidateMtf(c.symbol, rootTf, confirmMaxWait);
