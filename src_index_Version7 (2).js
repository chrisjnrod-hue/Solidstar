// src/index.js — minimal & focused scanner:
// - strict root-current-candle flip + flexible MTF rules
// - require vol24% > 0 AND latestHist > 0 to be eligible for trade opens
// - rank eligible by latestHist descending and open up to MAX_OPEN_TRADES
// - debug endpoints included; unused code removed for clarity
...
console.log('CONFIG', { AUTO_SCAN_ROOT_TFS, REST_CONCURRENCY, REST_THROTTLE_MS, MAX_SCAN_SYMBOLS, POLL_INTERVAL_MS, MAX_WATCHLIST, MACD_EPSILON, MACD_DYNAMIC_EPS_MULT, CONFIRM_CONCURRENCY, HIST_LOOKBACK });
...
async function fallbackRootScan(rootTf='1h', symbolLimit=MAX_SCAN_SYMBOLS) {
  ...
  // immediate flips: root flip on current or recent series crossing (HIST_LOOKBACK)
...
// ---------------- pollWatchlistForFlips (used by performRootScan) ----------------
...
async function performRootScan(rootTf='1h', symbolLimit=MAX_SCAN_SYMBOLS) {
  ...
  // compute simple strength (we'll only use latestHist ranking for eligibility/ordering)
  ...
  // === Enforce rule: only volChangePct > 0 paired with strongest MACD hist are eligible ===
  ...
  // Attempt open (subject to OPEN_TRADES)
...
if (PUSH_ALL_SIGNALS_ON_STARTUP) {
  ...
  })().catch(e => console.warn('PUSH_ALL_SIGNALS_ON_STARTUP error', e && e.message));
}
...
app.get('/debug/test-telegram', async (req, res) => {
  try {
    ensureRuntime();
    const message = req.query.message || `Scanner test message ${new Date().toISOString()}`;
