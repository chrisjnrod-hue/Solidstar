...
async function fetchKlines(symbol, interval, limit = 200) {
  ...
  const base = (process.env.BYBIT_BASE_API || 'https://api.bybit.com').replace(/\/$/,'');
  const url = `${base}/v5/market/kline?category=linear&symbol=${encodeURIComponent(symbol)}&interval=${encodeURIComponent(interval)}&limit=${Number(limit)}`;
    ...
    if (payload && payload.result && Array.isArray(payload.result.list)) {
      ...
      return list.map(item => {
        if (Array.isArray(item)) {
          ...
          return {
            timestamp: item.t || item.ts || item.open_time,
...
async function fetchUsdtPerpetualSymbols() {
  ...
  // HTTP fallback: query instruments-info and extract trading USDT perpetual symbols
  const base = (process.env.BYBIT_BASE_API || 'https://api.bybit.com').replace(/\/$/,'');
  const url = `${base}/v5/market/instruments-info?category=linear`;
  ...
  try {
    ...
    // Normalize: find symbol property, filter to USDT perpetuals
    const syms = list
      .map(it => (it && (it.symbol || it.name || it.code)) || '')
      .filter(Boolean)
      .filter(s => /USDT$/i.test(s)); // prefer USDT tickers
