// src/index.js — websocket-enabled confirm + 5m scheduler + ascending-hist filter
// Note: minimal, targeted changes only — rest of the code kept as-is.
...
//    * positiveAny: any positive histogram values in recent histSample (now tolerant using dynamic eps)
...
// ---------------- performRootScan (unchanged logic) ----------------
...
// ---------------- scheduler helpers (updated: run scans on every 5m candle open) ----------------
...
function startFiveMinScheduler() {
  ...
  setTimeout(async function first() {
    ...
    } catch (e) { console.warn('5min Scheduler initial run failed', e && e.message); }
...
// Start the 5-minute scheduler instead of per-root aligned intervals
