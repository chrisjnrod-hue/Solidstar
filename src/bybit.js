/* Bybit WS improvements: better diagnostics for unexpected-response (handshake) failures,
   ...
   This version adds optional verbose logging for subscribe messages and incoming responses
   when cfg.WS_VERBOSE === true, to help debug subscribe errors and unexpected closures.
*/
...
class BybitWS {
    ...
    try {
      ...
      this.ws.on('message', (d) => {
        try {
          ...
          // optional verbose dump for messages that look like subscription responses or errors
          if (cfg.WS_VERBOSE) {
            // do not flood logs – only log subscription-related responses and top-level keys helpful for debugging
            const isSubResp = msg && (msg.request || msg.success === false || msg.retCode || msg.ret_code || msg.topic);
            if (isSubResp) {
              try { console.log('BybitWS (verbose) recv:', JSON.stringify(msg).slice(0, 2000)); } catch (e) {}
            }
          }
      ...
      this.ws.on('close', (code, reason) => {
        ...
        console.warn(`BybitWS closed (code=${code}) reason=${reason ? (reason.toString ? reason.toString() : String(reason)) : ''} - reconnecting in ${ms}ms (attempt ${reconnectAttempts})`);
  ...
  subscribeKline(symbol, interval, cb) {
    // topic format: klineV2.<interval>.<symbol> (older versions) — keep for legacy, but v5 topics differ.
