// src/index.js
// Updated: combined /debug/test-telegram (accepts GET and POST), improved MACD, sorted klines,
// tolerant telegram calling, and debug endpoints.
...
const { createLimiter, sleep } = require('./utils'); // src/utils.js (must exist)
...
// --- added: safe numeric helpers and strict flip / mtf acceptance helpers ---
function toNumberSafe(v) {
  const n = Number(v);
  return Number.isFinite(n) ? n : 0;
}

// strict root flip: previous closed histogram negative and current histogram positive
function isStrictNegToPos(prev, cur) {
  const p = toNumberSafe(prev);
  const c = toNumberSafe(cur);
  return (p < 0) && (c > 0);
}

// MTF acceptance rules:
// - For non-1d mtfs (5m,15m,4h): accept if currentHist > 0
// - For 1d: accept if currentHist > 0 OR currentHist > prevClosedHist (rising allowed)
function isMtfPass(mtf, prev, cur) {
  const p = toNumberSafe(prev);
  const c = toNumberSafe(cur);
  if (String(mtf).toLowerCase() === '1d') {
    return (c > 0) || (c > p);
  }
  return (c > 0);
}
...
async function fallbackRootScan(rootTf='1h', symbolLimit=MAX_SCAN_SYMBOLS) {
  ...
  const immediateFlips = results.filter(r => isStrictNegToPos(r.prevClosedHist, r.currentHist));
...
async function confirmCandidateMtf(symbol, rootTf, maxWaitSeconds) {
  // New behavior:
  // - root flip detected externally (prevClosed < 0 && current > 0)
  // - For each MTF in the map, accept if:
  //     * mtf != '1d'  => currentHist > 0
  //     * mtf == '1d'  => currentHist > 0 OR (currentHist > prevClosedHist)  // rising allowed
  // - If 5m or 15m are negative at first check, poll until timeout for flips (strict neg->pos).
  // - If any MTF flips (strict neg->pos) during polling, re-evaluate and accept if the rules are satisfied.
  ...
  // fetch current mtf hist state and compute whether it initially passes the MTF rule
  async function checkMtfState(sym, mtf) {
    try {
      // map mtf string to interval used by fetchHistPrevAndCur
      const interval = (mtf === '5m' ? '5' : (mtf === '15m' ? '15' : (mtf === '1h' ? '60' : (mtf === '4h' ? '240' : (mtf === '1d' ? 'D' : mtf)))));
      const h = await fetchHistPrevAndCur(sym, interval, 30);
      if (!h) return { mtf, pass: false, reason: 'no-hist', prev: null, cur: null, lastKTime: 0 };
      ...
      // Rule: for 1d accept positive OR rising; for others accept positive only
      let pass = false;
      if (String(mtf).toLowerCase() === '1d') {
        pass = (cur > 0) || (cur > prev);
      } else {
      ...
      return { mtf, pass: !!pass, prev, cur, lastKTime, flipped: isStrictNegToPos(prev, cur) };
    } catch (e) {
      return { mtf, pass: false, reason: e && e.message, prev: null, cur: null, lastKTime: 0 };
    }
  }

  // Main polling loop: try immediately, then poll until deadline
  let lastDetails = [];
  const pollIntervalMs = Math.max(1000, Math.min(5000, POLL_INTERVAL_MS || 3000)); // 1s..5s bounded
  while (Date.now() < deadline) {
    // fetch states for all mtfs in parallel (rate-limited by REST_THROTTLE_MS)
    ...
    // If all pass according to the rules, confirm immediately
    const allPass = checks.length > 0 && checks.every(c => !!c && c.pass);
    if (allPass) {
      return { confirmed: true, details: checks };
    }

    // If not all pass, see if any of the MTFs that are negative have just flipped (strict neg->pos)
    const anyFlipped = checks.some(c => !!c && !!c.flipped);
    if (anyFlipped) {
      // Re-evaluate: if rules now satisfied, confirm
      const reAllPass = checks.every(c => !!c && c.pass);
      if (reAllPass) return { confirmed: true, details: checks };
      // else continue waiting until timeout (other mtfs might still be negative)
    }

    // If 5m or 15m are negative and not yet flipped, we need to wait/poll (user requested)
    // (we don't short-circuit here; we keep polling until deadline)
    const now = Date.now();
  ...
  while (Date.now() < endTs && polledSet.size > 0) {
    const tasks = Array.from(polledSet).map(sym => limiter(async () => {
      try {
        ...
        if (isStrictNegToPos(prevClosedHist, currentHist)) {
...
// ---------------- breakevenManager (kept unchanged — ensure yours is present) ----------------
...
async function performRootScan(rootTf='1h', symbolLimit=MAX_SCAN_SYMBOLS) {
  ...
  for (const r of scanResults) {
    ...
    if (isStrictNegToPos(r.prevClosedHist, r.currentHist)) immediateFlips.push(r);
