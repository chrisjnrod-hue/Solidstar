// src/index.js — updated: configurable lookback, startup push, current_roots debug routes,
// streaming confirmations, robust fetch + timestamps, and improved detection logging.
// Fixed: replaced fragile nested ternaries with tfToIntervalKey helper to avoid SyntaxError
...
console.log('CONFIG', { AUTO_SCAN_ROOT_TFS, REST_CONCURRENCY, REST_THROTTLE_MS, MAX_SCAN_SYMBOLS, ENABLE_ROOT_POLLING, POLL_INTERVAL_MS, MAX_WATCHLIST, MACD_EPSILON, MACD_DYNAMIC_EPS_MULT, CONFIRM_CONCURRENCY, HIST_LOOKBACK, PUSH_ALL_SIGNALS_ON_STARTUP, MACD_THRESHOLD, NEG_ALLOWED_IN_MTF, TRADE_ON_CONFIRM_NOW });
...
// This function implements MTF confirmation logic per user's specs:
// - For each mtf in MTF_CHECK_MAP[rootTf]:
//    * positiveAny: any positive histogram values in recent histSample
//    * flipped: strict negative->positive flip on current candle (isNegToPosFlexible)
//    * For '1d' we accept rising negatives (cur > prev)
// - Quick confirm if all mtfs have positiveAny (or rising1d).
// - Else allow up to NEG_ALLOWED_IN_MTF mtfs to be still-negative but require those frames to have flipped === true.
// - lastToFlip: the timeframe (mtf) that flipped most recently (max lastKTime) among those with flipped==true
...
async function confirmCandidateMtf(symbol, rootTf, maxWaitSeconds) {
  ...
  async function checkMtfState(sym, mtf) {
    try {
      ...
      const positiveAny = Array.isArray(h.histSample) && h.histSample.some(x => Number(x && x.hist) > 0);
      ...
      return { mtf, pass: !!passBasic, prev, cur, lastKTime, histSample: h.histSample, flipped, positiveAny, rising1d };
    } catch (e) {
      return { mtf, pass: false, reason: e && e.message, prev: null, cur: null, lastKTime: 0, histSample: [] };
    }
  }

  // Quick immediate check — confirm instantly if all mtfs already pass with positiveAny/rising1d
  try {
    const quickChecks = await Promise.all(mtfs.map(mtf => checkMtfState(symbol, mtf)));
    const allQuickPass = quickChecks.length > 0 && quickChecks.every(c => !!c && (c.positiveAny || c.rising1d || (c.cur > 0)));
  ...
  while (Date.now() < deadline) {
    ...
    // - If every mtf has positiveAny or rising1d or cur>0 -> confirm
    // - Else, allow up to NEG_ALLOWED_IN_MTF mtfs to be "not positiveAny/rising1d", but require those frames to have flipped === true (current-candle flip)
    const failing = checks.filter(c => !(c.positiveAny || c.rising1d || (c.cur > 0)));
    if (failing.length === 0) {
      // all good
    ...
    if (failing.length <= NEG_ALLOWED_IN_MTF) {
      const allFailingFlipped = failing.every(c => !!c.flipped);
    ...
    // Not confirmed yet. If any flipped recently, we can re-evaluate quickly in next iteration.
    const now = Date.now();
...
async function performRootScan(rootTf='1h', symbolLimit=MAX_SCAN_SYMBOLS) {
  ...
  confirmedCandidates = confirmedCandidates.filter(c => {
    const volOk = Number(c.volChangePct || 0) > 0;
    const macdOk = Number(c.latestHist || 0) > Number(MACD_THRESHOLD || 0);
    if (!volOk) {
      if (process.env.DEBUG_DETECTION === 'true') console.debug('Reject candidate due to non-positive 24h vol', c.symbol, c.volChangePct);
    }
    if (!macdOk) {
      if (process.env.DEBUG_DETECTION === 'true') console.debug('Reject candidate due to macd threshold', c.symbol, c.latestHist, MACD_THRESHOLD);
    }
    return volOk && macdOk;
  });
  ...
  if (confirmedCandidates.length) {
    const lines = confirmedCandidates.map((c,i) => `${i+1}. ${c.symbol} ${c.filterLabel ? '['+c.filterLabel+'] ' : ''}score=${(c.strengthScore||0).toFixed(8)} macd=${(c.latestHist||0).toFixed(8)} vol24%=${(c.volChangePct||0).toFixed(2)} lastToFlip=${c.lastToFlip||''}`);
    await sendNotice('mtf', `MTF confirmed (${rootTf}) count=${confirmedCandidates.length}`, lines);
  } else {
    await sendNotice('mtf', `MTF confirmed (${rootTf}) none`, ['No candidates passed MTF within root window (filtered for vol>0 & MACD_THRESHOLD)']);
  }
    ...
    for (const c of toOpen) {
      try {
        ...
        if (TRADE_ON_CONFIRM_NOW && tradeModule && typeof tradeModule.openTrade === 'function' && OPEN_TRADES) {
          ...
          await sendNotice('trade-opened', `Trade opened ${c.symbol}`, [`result: ${JSON.stringify(result)}`, `score=${(c.strengthScore||0).toFixed(8)}`]);
          if (ENABLE_BREAKEVEN_SIGNAL) {
            const tradeInfo = {
          ...
          await sendNotice('trade-dryrun', `DRY-RUN trade for ${c.symbol}`, [`score=${(c.strengthScore||0).toFixed(8)} label=${c.filterLabel||''}`, `note=TRADE_ON_CONFIRM_NOW=${TRADE_ON_CONFIRM_NOW}`]);
        }
      } catch (e) {
...
app.get('/debug/macd', async (req, res) => {
  ...
  try {
    ...
    const interval = tfToIntervalKey(tf);
