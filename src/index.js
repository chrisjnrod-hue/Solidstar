// src/index.js — updated: robust symbol discovery fallback, MTF summary, macd higher-highs+volume strength,
// rootTf included in confirmations, dedupe retained.
...
async function fallbackRootScan(rootTf='1h', symbolLimit=MAX_SCAN_SYMBOLS) {
  ...
  if (by && typeof by.fetchUsdtPerpetualSymbols === 'function') {
    try {
      ...
      if (Array.isArray(syms) && syms.length) {
        return await _scanSymbolsFromList(rootTf, syms.slice(0, Number(symbolLimit || MAX_SCAN_SYMBOLS)));
      }
    } catch (e) {
  ...
  try {
    ...
    const syms = list.map(it => (it && (it.symbol || it.name || it.code)) || '').filter(Boolean).slice(0, Number(symbolLimit || MAX_SCAN_SYMBOLS));
    return await _scanSymbolsFromList(rootTf, syms);
  } catch (e) {
...
async function performRootScan(rootTf='1h', symbolLimit=MAX_SCAN_SYMBOLS) {
  ...
  const scanResults = await fallbackRootScan(canonicalTf, symbolLimit).catch(e => {
