// src/config.js (edited to auto-select TESTNET when not explicitly set)
...
// AUTO-SELECT TESTNET
// Priority: env.TESTNET explicit -> NODE_ENV heuristic -> default to testnet for non-production
let TESTNET;
if (env.TESTNET !== undefined) {
  TESTNET = parseBool(env.TESTNET, true);
} else {
  TESTNET = (env.NODE_ENV && String(env.NODE_ENV).toLowerCase() === 'production') ? false : true;
}
