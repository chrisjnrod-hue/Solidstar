// src/index.js — updated: strict MACD flip logic, watchlist built from root-scan (prevClosedHist < 0), mid-candle polling.
...
const POLL_INTERVAL_MS = Math.max(1000, Number(process.env.POLL_INTERVAL_MS || 10000)); // default 10s
const MAX_WATCHLIST = Math.max(1, Number(process.env.MAX_WATCHLIST || 50));
const MACD_EPSILON = Number(process.env.MACD_EPSILON || 0); // treat any positive if 0
...
console.log('CONFIG', { MTF_CHECK_MAP, AUTO_SCAN_ROOT_TFS, REST_CONCURRENCY, REST_THROTTLE_MS, MAX_SCAN_SYMBOLS, FILTER_SIGNALS, ENABLE_ROOT_POLLING, POLL_INTERVAL_MS, MAX_WATCHLIST, MACD_EPSILON });
...
async function fetchHistPrevAndCur(symbol, tfIntervalKey, klinesLimit = 100) {
  ...
  return { prevClosedHist, currentHist, lastK: klines[len - 1] };
}

// ---------------- fallbackRootScan (now returns hist info for candidates) ----------------
async function fallbackRootScan(rootTf='1h', symbolLimit=MAX_SCAN_SYMBOLS) {
  ...
  console.log('fallbackRootScan: scanned results count=', results.length);
  return results;
}
...
// ---------------- watchlist polling: monitor mid-candle flips ----------------
async function pollWatchlistForFlips(watchlist, rootTf, rootWindowSeconds) {
  ...
  const intervalKey = (rootTf === '1h') ? '60' : (rootTf === '4h' ? '240' : (rootTf === '1d' ? 'D' : rootTf));
  const limiter = createLimiter(Math.max(1, Math.min(REST_CONCURRENCY, 8))); // limit polling concurrency
  ...
  while (Date.now() < endTs && polledSet.size > 0) {
    // build tasks for current watchlist symbols
    ...
    for (const r of results) {
      if (r) {
        // record flip and remove from polledSet
        flipped.push(r);
    ...
    // remove any symbols exceeding TTL or if polledSet now empty
...
// (unchanged from previous version — omitted here for brevity in this block)
// Include your previously merged breakevenManager code here or keep existing implementation.
// For brevity, assume breakevenManager object exists and is same as earlier code.
...
// ---------------- performRootScan: immediate flips + watchlist polling -> MTF confirmation -> trade plan ----------------
async function performRootScan(rootTf='1h', symbolLimit=MAX_SCAN_SYMBOLS) {
  ...
  for (const r of scanResults) {
    ...
    if ((r.prevClosedHist < 0) && (r.currentHist > MACD_EPSILON)) {
      immediateFlips.push(r);
    } else if (r.prevClosedHist < 0) {
      watchCandidates.push(r);
    }
  }

  // Immediately push any immediate root flips
  ...
  // Build watchlist from watchCandidates (rank by abs(prevClosedHist) desc) and poll for flips
  let watchlist = [];
  if (ENABLE_ROOT_POLLING && watchCandidates.length) {
    ...
    // merge flipped polled entries into immediate array
    for (const f of flips) {
      // map to same shape as immediateFlips
      immediateFlips.push({ symbol: f.symbol, rootTf, prevClosedHist: f.prevClosedHist, currentHist: f.currentHist, lastVolume: 0 });
    }
  }

  // If no immediate flips found, still send a root notice summarizing none found if you prefer (optional)
  if (!immediateFlips.length && !watchlist.length) {
    // no immediate flips and polling not used/empty
    await sendNotice('root', `Root scan (${rootTf}) — no immediate flips`, [`scanned=${scanResults.length}`]);
  }

  // Now confirm MTF for each immediate flip (with concurrency limiter) and proceed to trade planning
  if (!immediateFlips.length) {
    lastScanResults[rootTf] = [];
    return [];
  }
  ...
  // trade plan + opens (same as earlier)
  if (confirmedCandidates.length) {
