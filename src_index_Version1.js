  ...
  try {
    ...
    for (const mtf of mtfs) {
      try {
        ...
        const positiveAny = Array.isArray(h.histSample) && h.histSample.some(x => Number(x && x.hist) > 0);
        const rising1d = (String(mtf).toLowerCase().startsWith('1d') || String(mtf).toLowerCase() === 'd') ? (cur > prev) : false;
        mtfDetails.push({ mtf, prev, cur, flipped, positiveAny, rising1d, sampleLen: (h.histSample && h.histSample.length) || 0 });
      } catch (e) {
    ...
    return res.json({
      ...
      macdOk,
      env: { MACD_THRESHOLD, NEG_ALLOWED_IN_MTF, HIST_LOOKBACK, FILTER_SIGNALS }
    });
  } catch (e) {
    return res.status(500).json({ ok:false, error: e && e.message });
  }
