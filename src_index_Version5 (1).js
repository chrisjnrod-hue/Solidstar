...
// streaming confirmations, robust fetch + timestamps, improved detection logging,
// and strict trade eligibility: volChangePct > 0 + strongest MACD hist prioritized.
...
// Strict flip-on-current-candle test (root must satisfy)
...
// Flexible negative->positive detection with optional HIST_LOOKBACK series-scan
  ...
  if (HIST_LOOKBACK && HIST_LOOKBACK > 0 && rawHist.length >= 2) {
    ...
    for (let i = 0; i < tail.length - 1; i++) {
      ...
      if (Number(a) < 0 && Number(b) > 0 && (b - a) >= minDelta) {
        if (process.env.DEBUG_DETECTION === 'true') console.debug('isNegToPosFlexible crossing', { idx: i, a, b, minDelta });
        return true;
      }
    }
  }
...
// small helper: check series crossing (controlled by HIST_LOOKBACK)
function histHasNegToPosCrossing(histSample, lookback = HIST_LOOKBACK, minDelta = 1e-8) {
  ...
  for (let i = 0; i < tail.length - 1; i++) {
    ...
    if (a < 0 && b > 0 && ((b - a) >= minDelta)) {
      if (process.env.DEBUG_DETECTION === 'true') console.debug('histHasNegToPosCrossing: detected', { idx: i, a, b });
      return true;
    }
  }
  return false;
...
// messaging helper (buffers notices and tries Telegram)
async function sendNotice(type, title, lines) {
  ...
  if (telegramModule) {
    ...
    } else console.log('sendNotice: telegram send function missing');
  }
  console.log(body);
}

// compute 24h volume % change (best-effort)
...
// robust fetch prev/current hist (returns prevClosedHist, currentHist, histSample with ts_ms)
...
// fallback root scan — builds per-symbol prev/cur/histSample
...
// confirmCandidateMtf implements your rules:
// - root TF must be flip-on-current-candle (strict)
// - other TFs accepted if cur>0 OR (1d and cur>prev)
// - negative other TFs are polled until they flip-on-current or timeout
async function confirmCandidateMtf(symbol, rootTf, maxWaitSeconds) {
  ...
  async function checkStateForMtf(sym, mtf) {
    try {
      ...
      const flexibleFlip = isNegToPosFlexible(prev, cur, h.histSample);
      return { mtf, prev, cur, lastKTime, histSample: h.histSample, passPositive, passDailyRising, flipCurrent, flexibleFlip };
    } catch (e) { return { mtf, pass:false, reason: e && e.message, prev: null, cur: null, lastKTime: 0, histSample: [] }; }
  }
  ...
  try {
    ...
    if (!rootStatus || !rootStatus.flipCurrent) {
      return { confirmed: false, details: quick, reason: 'root-not-current-flip' };
    }
    const othersOk = quick.every(q => {
      if (String(q.mtf).toLowerCase() === String(rootTf).toLowerCase()) return true;
      if ((q.passPositive === true) || (q.passDailyRising === true)) return true;
      return false;
    });
...
// pollWatchlistForFlips (unchanged)
...
// breakeven manager (unchanged; kept for completeness)
...
async function performRootScan(rootTf='1h', symbolLimit=MAX_SCAN_SYMBOLS) {
  ...
  // === Trade planning and open selection: enforce vol>0 + strongest MACD first ===
  // Filter confirmed candidates: require positive 24h volChangePct and positive latestHist
  const eligible = confirmedCandidates
    .filter(c => Number(c.latestHist || 0) > 0 && Number(c.volChangePct || 0) > 0);

  // Sort eligible by latestHist descending (largest MACD histogram first)
  eligible.sort((a,b) => (Number(b.latestHist || 0) - Number(a.latestHist || 0)));

  // Candidates excluded (for transparency) — either non-positive vol or non-positive hist
  const excluded = confirmedCandidates.filter(c => !(Number(c.latestHist || 0) > 0 && Number(c.volChangePct || 0) > 0));

  // Prepare toOpen from eligible up to MAX_OPEN_TRADES
  const toOpen = eligible.slice(0, MAX_OPEN_TRADES);

  // Notify planned opens and excluded for transparency
  if (toOpen.length) {
    const planLines = [];
    for (const c of toOpen) {
      ...
      planLines.push(`${c.symbol} score=${(c.strengthScore||0).toFixed(3)} macd=${(c.latestHist||0).toFixed(8)} vol24%=${(c.volChangePct||0).toFixed(2)} suggestedSize=${suggested} label=${c.filterLabel||''}`);
    }
  ...
  // Proceed to open trades only for toOpen (dry-run or actual depending on OPEN_TRADES)
  for (const c of toOpen) {
    try {
      if (tradeModule && typeof tradeModule.openTrade === 'function' && OPEN_TRADES) {
        ...
        await sendNotice('trade-dryrun', `DRY-RUN trade for ${c.symbol}`, [`score=${(c.strengthScore||0).toFixed(3)} macd=${(c.latestHist||0).toFixed(8)} vol24%=${(c.volChangePct||0).toFixed(2)} label=${c.filterLabel||''}`]);
      }
    } catch (e) {
